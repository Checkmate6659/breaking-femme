...
public class AndrostadienedioneOilSolutionCauldronBlock extends AbstractCauldronBlock {
    ...
    static {
        CauldronBehavior FILL = (state, world, pos, player, hand, stack) -> {
            return CauldronBehavior.fillCauldron(world, pos, player, hand, stack, ModFluids.ANDROSTADIENEDIONE_OIL_SOLUTION_CAULDRON.getDefaultState().with(LEVEL, 3), SoundEvents.ITEM_BUCKET_EMPTY);
        };

        //vanilla fluids
        CauldronBehavior.EMPTY_CAULDRON_BEHAVIOR.put(ModFluids.ANDROSTADIENEDIONE_OIL_SOLUTION_BUCKET, FILL);
        CauldronBehavior.WATER_CAULDRON_BEHAVIOR.put(ModFluids.ANDROSTADIENEDIONE_OIL_SOLUTION_BUCKET, FILL);
        CauldronBehavior.LAVA_CAULDRON_BEHAVIOR.put(ModFluids.ANDROSTADIENEDIONE_OIL_SOLUTION_BUCKET, FILL);
        ...
        BEHAVIOR.put(Items.GLASS_BOTTLE, (state, world, pos, player, hand, stack) -> {
            if (!world.isClient) {
                ...
                player.setStackInHand(hand, ItemUsage.exchangeStack(stack, player, new ItemStack(ModItems.ANDROSTADIENEDIONE_OIL_BOTTLE)));
        ...
        CauldronBehavior.EMPTY_CAULDRON_BEHAVIOR.put(ModItems.ANDROSTADIENEDIONE_OIL_BOTTLE, (state, world, pos, player, hand, stack) -> {
            if (!world.isClient) {
                Item item = stack.getItem();
                player.setStackInHand(hand, ItemUsage.exchangeStack(stack, player, new ItemStack(Items.GLASS_BOTTLE)));
                player.incrementStat(Stats.USE_CAULDRON);
                player.incrementStat(Stats.USED.getOrCreateStat(item));
                world.setBlockState(pos, ModFluids.ANDROSTADIENEDIONE_OIL_SOLUTION_CAULDRON.getDefaultState());
        ...
        BEHAVIOR.put(ModItems.ANDROSTADIENEDIONE_OIL_BOTTLE, (state, world, pos, player, hand, stack) -> {
    ...
    public AndrostadienedioneOilSolutionCauldronBlock(AbstractBlock.Settings settings) {
