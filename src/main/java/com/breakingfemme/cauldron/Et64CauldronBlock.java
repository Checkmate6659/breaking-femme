...
public class Et64CauldronBlock extends AbstractCauldronBlock {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if(random.nextInt(3) == 0) return; //evaporation speed 2/3 that of 95%
        ...
        {
            //ethanol boils off before water does, so make less concentrated solution
            world.setBlockState(pos, ModFluids.ET32_CAULDRON.getDefaultState().with(LEVEL, state.get(LEVEL)));
            decrementFluidLevel(world.getBlockState(pos), world, pos); //decrement the newly placed 32% ethanol cauldron
        }
    }
