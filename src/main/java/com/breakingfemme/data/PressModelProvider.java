...
public abstract class PressModelProvider implements DataProvider {
    private static final class PressModelVariantProvider extends VariantsBlockStateSupplier {

        PressModelVariantProvider(Block block, List<BlockStateVariant> variants) {
        ...
        public PressModelVariantProvider coordinate(BlockStateVariantMap map) {
            this.breakingfemme$getVariantMaps().add(map);
            return this;
        }
    }
    ...
    public CompletableFuture<?> run(DataWriter writer) {
        ...
        return CompletableFuture.allOf(variantMap.entrySet().stream().map(entry -> {
            ...
            final VariantsBlockStateSupplier variantSupplier = PressModelVariantProvider.create(null).coordinate(map);
