...
void main() {
    if(BlurStrength == 0.0) //be able to disable shader
        fragColor.xyz = texture(DiffuseSampler, texCoord).xyz;
    else
    {
        vec2 dirvector = vec2(Direction, 1.0 - Direction);
        float invstd2 = 1.0 / (BlurStrength * BlurStrength); //invstd2 is sigma^-2, precomputed for efficiency
        ...
        {
            float weight = exp(-r*r * 0.5 * invstd2); //constant 1/(std * sqrt(2pi)) is NOT required due to normalization
